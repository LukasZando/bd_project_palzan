...
from pyspark.sql.functions import to_date, col, count, date_format, to_utc_timestamp, to_timestamp, concat_ws, udf
...
df = df.withColumn("violation_code", col("violation_code").cast(StringType()))
df = df.withColumnRenamed("Plate Type", "plate_type")
...
# filter unusable plate data
df = df.filter(df["plate_id"].isNotNull())
df = df.filter(df["plate_id"] != "BLANKPLATE")
...
df = drop_duplicates_based_on_columns(df, ["plate_id", "street_name", "violation_code", "violation_iso_timestamp"])

df = df.select(["plate_id", "street_name", "violation_code", "plate_type", "vehicle_make", "violation_iso_timestamp"])

##################################
## Total offenders (used as baseline for comparison)
##################################
...
df.write.format("org.elasticsearch.spark.sql") \
    ...
    .mode("overwrite") \
    .save("offenders_total_new")

##################################
## Repeated offenders (5 or more violations)
##################################

# Filter plates with 5 or more violations
plate_counts = df.groupBy("plate_id").agg(count("*").alias("violation_count"))
frequent_offenders = plate_counts.filter(col("violation_count") >= 5)
df = df.join(frequent_offenders, on="plate_id", how="inner")

print("Repeated offenders to write:", df.count())
